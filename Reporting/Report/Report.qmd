...
```{r incidence 1, echo=FALSE, message = FALSE, warning = FALSE}
inc <- data[["incidence"]] |>
  dplyr::filter(estimate_name == "incidence_100000_pys") |>
  omopgenerics::splitAdditional() |>
  omopgenerics::splitGroup() |>
...
ggplot2::ggplot(inc2, ggplot2::aes(x = incidence_start_date, y = estimate_value, colour = denominator_age_group, group = denominator_age_group)) +
  ggplot2::geom_point() +
  ggplot2::geom_line() +
  ggplot2::facet_wrap(~ cdm_name, scales = "free") +
  ggplot2::theme(axis.text.x = ggplot2::element_blank()) 
